...
const addEntryToDb = (storeName, entry) => {
  onRequest.result.transaction([storeName], "readwrite").objectStore(storeName).add(entry);
};

// clearAllEntries 함수를 작성합니다.
const clearAllEntries = (storeName) => {
  console.log("삭제진행");
  onRequest.result.transaction([storeName], "readwrite").objectStore(storeName).clear();
};
